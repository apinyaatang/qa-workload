    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h2 className="text-base font-bold text-gray-900">Master Staff</h2>
            <p className="text-sm text-gray-400 mt-0.5">จัดการข้อมูลพนักงาน ({employees.length} คน)</p>
          </div>
          <div className="flex items-center gap-2">
            {/* Import CSV button */}
            <label className="flex items-center gap-2 px-4 py-2 border border-indigo-300 text-indigo-600 rounded-lg text-sm font-medium hover:bg-indigo-50 transition-colors cursor-pointer">
        ...
        <div className="mt-4 flex gap-3 flex-wrap">
          ...
          <select value={filterDept} onChange={e => setFilterDept(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none">
            <option value="all">ทุกแผนก</option>
            {depts.map(d => <option key={d} value={d}>{d}</option>)}
          </select>
          <select value={filterActive} onChange={e => setFilterActive(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map(emp => {
                ...
                return (
                  <>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1 max-w-[200px]">
                          {emp.skills.slice(0, 3).map(s => (
                            <span key={s} className="text-xs bg-slate-50 text-slate-600 px-1.5 py-0.5 rounded border border-slate-100">{s}</span>
                          ))}
                        ...
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {deleteConfirm === emp.id ? (
                            <span className="flex items-center gap-1">
                              <button onClick={() => { deleteEmployee(emp.id); setDeleteConfirm(null) }} className="p-1.5 text-red-600 hover:bg-red-50 rounded">
                                <Check size={14} />
                              </button>
                            ...
                            <button onClick={() => setDeleteConfirm(emp.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded">
                              <Trash2 size={14} />
                            </button>
                          )}
                        </div>
                    ...
                    {isExpanded && (
                      <tr key={emp.id + '-leave'}>
                        <td colSpan={7} className="bg-gray-50/80 px-5 py-4 border-t border-gray-100">
                          <div className="space-y-3">
                            <p className="text-xs font-semibold text-gray-600">บันทึกวันลา — {emp.firstName} {emp.lastName}</p>
                            ...
                            {empLeaves.length > 0 ? (
                                ...
                                {empLeaves.map(l => (
                                  <div key={l.id} className="flex items-center justify-between bg-white border border-gray-100 rounded-lg px-3 py-2">
                                    <div className="flex items-center gap-3">
                                      <span className="text-sm text-gray-700 font-medium">{formatDate(l.date)}</span>
                                      <span className="text-xs text-gray-500">{LEAVE_TYPES.find(t => t.value === l.leaveType)?.label}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                      ...
                                      {l.status === 'pending' && (
                                        <span className="flex items-center gap-1">
                                          <button onClick={() => updateLeaveStatus(l.id, 'approved')} className="p-1 text-green-600 hover:bg-green-50 rounded text-xs" title="อนุมัติ">
                                            <Check size={12} />
                                          </button>
                                          <button onClick={() => updateLeaveStatus(l.id, 'rejected')} className="p-1 text-red-500 hover:bg-red-50 rounded text-xs" title="ไม่อนุมัติ">
                                            <X size={12} />
                                          </button>
                                        </span>
                                      )}
                              ...
                              <p className="text-xs text-gray-400">ยังไม่มีการบันทึกวันลา</p>
                            )}
                          </div>
                        </td>
                      </tr>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={7} className="px-5 py-12 text-center text-gray-400">ไม่พบข้อมูลพนักงาน</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
              <h3 className="font-bold text-gray-900">{editId ? 'แก้ไขข้อมูลพนักงาน' : 'เพิ่มพนักงานใหม่'}</h3>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-start justify-center z-50 pt-12 px-4 pb-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="font-bold text-gray-900">Import Staff จาก CSV</h3>
            ...
            <div className="p-6">
              {/* Parse error */}
              {importParseError && (
                <div className="mb-4 flex items-start gap-2 p-3 bg-red-50 rounded-lg text-red-700 text-sm">
              ...
              {importDone ? (
                <div className="text-center py-8 space-y-3">
                  ...
                  <p className="text-lg font-semibold text-gray-900">Import สำเร็จ</p>
                  <div className="flex justify-center gap-6 text-sm text-gray-600">
                    <span className="flex flex-col items-center">
                      <span className="text-2xl font-bold text-green-600">{importDone.inserted}</span>
                      เพิ่มใหม่
                    </span>
                    <span className="flex flex-col items-center">
                      <span className="text-2xl font-bold text-blue-600">{importDone.updated}</span>
                      อัปเดต
                    </span>
                  </div>
                  <button
                ...
                <>
                  ...
                  {/* Preview table */}
                  <div className="overflow-x-auto max-h-80 rounded-lg border border-gray-100">
                    <table className="w-full text-xs">
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {importRows.map(row => (
                          <tr key={row.rowNo} className={row.isValid ? '' : 'bg-red-50'}>
                            <td className="px-3 py-1.5 font-mono text-gray-500">{row.data.employeeCode}</td>
                            <td className="px-3 py-1.5 font-medium text-gray-900">
                              {row.data.firstName} {row.data.lastName}
                            </td>
                            <td className="px-3 py-1.5 text-gray-500">{row.data.nickname}</td>
                            <td className="px-3 py-1.5 text-gray-600">{row.data.position}</td>
                            <td className="px-3 py-1.5 text-gray-600">{row.data.group}</td>
                            <td className="px-3 py-1.5">
                              <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${row.data.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                                {row.data.isActive ? 'Active' : 'Inactive'}
                              </span>
                            </td>
                            <td className="px-3 py-1.5">
                              {row.isValid ? (
                                <span className={`text-[10px] font-medium ${row.willUpdate ? 'text-blue-600' : 'text-green-600'}`}>
                ...
                <p className="text-center text-gray-400 text-sm py-8">เลือกไฟล์ CSV เพื่อ preview ข้อมูล</p>
              )}
            </div>
          </div>
        </div>
