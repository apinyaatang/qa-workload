      ...
      <div className="w-56 shrink-0 space-y-3">
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-50">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Master Data</p>
          </div>
          <nav className="p-2 space-y-0.5">
            {tabs.map(t => (
              ...
              >
                <span className={activeTab === t.key ? 'text-indigo-600' : 'text-gray-400'}>{t.icon}</span>
                <div className="flex-1 min-w-0">
                  <p className="font-medium leading-none">{t.label}</p>
                  <p className="text-xs text-gray-400 mt-0.5 truncate">{t.desc}</p>
                </div>
                {activeTab === t.key && <ChevronRight size={14} className="text-indigo-400 shrink-0" />}
              </button>
            ))}
          </nav>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-2">
          <p className="text-xs font-semibold text-gray-500">ข้อมูล Demo</p>
          ...
          {!confirmReset ? (
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 px-3 py-2 border border-gray-200 text-gray-500 rounded-lg text-xs hover:bg-gray-50 transition-colors"
            >
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-1.5">
                ...
                <button onClick={() => setConfirmReset(false)}
                  className="flex-1 px-2 py-1.5 border border-gray-200 text-gray-500 rounded text-xs hover:bg-gray-50">
                  ยกเลิก
                </button>
              </div>
            </div>
