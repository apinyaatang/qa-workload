        ...
        return (
          <div
            ...
            className={`rounded-lg border shadow-sm overflow-hidden ${
              isUnassigned ? 'border-gray-200' : workloadBgColor(group.totalEstimate)
            }`}
          >
            {/* Card header */}
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <div className="flex items-center gap-2 min-w-0">
                  <User
                    size={16}
                    className={`shrink-0 ${
                      isUnassigned ? 'text-gray-400' : workloadTextColor(group.totalEstimate)
                    }`}
                  />
                ...
                <div className="ml-auto flex items-center gap-3 shrink-0">
                  <span
                    className={`text-sm font-bold whitespace-nowrap ${
                      isUnassigned ? 'text-gray-500' : workloadTextColor(group.totalEstimate)
                    }`}
                  >
            ...
            <div className="divide-y divide-gray-100">
              {group.projects.map(p => (
                <div
                  key={p.id}
                  className="flex items-center gap-3 px-4 py-2 bg-white/50 hover:bg-white/80 transition-colors flex-wrap"
                >
                  {/* Priority badge */}
                  {p.priority && (
                    <span
                  ...
                  <div className="flex items-center gap-3 shrink-0 text-[11px] text-gray-500">
                    <span title="Test Date">
                      Test: <span className="text-gray-700">{formatDate(p.testDate)}</span>
                    </span>
                    <span title="UAT Date">
                      UAT: <span className="text-gray-700">{formatDate(p.uatDate)}</span>
                    </span>
